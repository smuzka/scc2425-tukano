...
public class AzureManagement {
	...
	static final String MY_ID = "71848"; // Add your suffix here
	...
	public static void main(String[] args) {
		try {
			...
			if (args.length == 1 && args[0].equalsIgnoreCase("--delete")) {
				System.out.println("Deleting Azure resources...");
				Arrays.stream(AZURE_RG_REGIONS).forEach(reg -> deleteResourceGroup(azure, reg));
				System.out.println("Azure resources deleted with success");
			} else {
				...
				if (CREATE_COSMOSDB) {
					Thread th = new Thread(() -> {
						try {
							...
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "following", "/id", null);
